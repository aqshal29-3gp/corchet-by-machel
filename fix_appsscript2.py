...
if not active:
    ...
    version = service.projects().versions().create(scriptId=script_id, body={'description': 'Auto deploy API fix'}).execute()
    created = service.projects().deployments().create(scriptId=script_id, body={
        'versionNumber': version['versionNumber'],
        'manifestFileName': 'appsscript',
        'description': 'Auto deploy API fix'
    }).execute()
    active = created

version = service.projects().versions().create(scriptId=script_id, body={'description': 'Auto deploy via API'}).execute()
updated = service.projects().deployments().update(
    scriptId=script_id,
    deploymentId=active['deploymentId'],
    body={'deploymentConfig': {'versionNumber': version['versionNumber'], 'manifestFileName': 'appsscript', 'description': 'Auto deploy via API'}}
).execute()
print("Deployed.")
web_app_url = ""
